...
Gateway -> MES: gửi realtime OEE snapshot
```
...
### 2.2 Start Hai Lệnh Trên Cùng Một Máy
...
## 4. Quy Tắc Tính Realtime Hiện Tại

- Gateway đọc PLC/OPC UA và lưu latest values trong RAM.
- Theo chu kỳ `MesSync.SyncIntervalMs`, Gateway capture raw interval vào SQLite `oee.db`.
- Mỗi active/pause session có baseline riêng trong `production_context`.
- `actual_qty = (shot_ok_total + shot_ng_total) hiện tại - baseline`.
- `planned_qty = production_time / cycle_time`.
- `cycle_time` lấy từ product trong production command.
- `production_time = now - active_period_start_at`.
- A/P/Q/OEE gửi theo phần trăm `0..100`.
- Nếu session chưa có baseline, Gateway seed baseline và bỏ qua snapshot đầu tiên của session đó.

## 5. Signal Code Gateway Cần Cấu Hình

Gateway map signal theo `SignalCode`, không phân biệt hoa thường:

| SignalCode | Ý nghĩa | Đơn vị |
|---|---|---|
| `MACHINE_STATE` | Trạng thái máy hiện tại | code/string |
| `SHOT_OK_COUNT` | Counter OK tích lũy từ PLC | count |
| `SHOT_NG_COUNT` | Counter NG tích lũy từ PLC | count |
| `CYCLE_TIME_MS` | Cycle time hiện tại | millisecond |
| `RUN_TIME_TOTAL` | Runtime tích lũy | second |
| `STOP_TIME_TOTAL` | Stop time tích lũy | second |
| `ERROR_TIME_TOTAL` | Error time tích lũy | second |

## 6. Test Nhanh Control API Bằng PowerShell
...
Invoke-RestMethod `
  -Uri "http://localhost:8080/api/v1/gateway/oee/production-commands" `
  -Method Post `
  -ContentType "application/json" `
  -Body $body
```

## 7. Diagnostic API Gateway

Gateway hiện có endpoint kiểm tra trạng thái sync:

```http
GET http://<gateway-host>:8080/api/v1/mes-sync/status
```
...
{
  "enabled": true,
  "last_success_unix_seconds": 1788750000,
  "last_error": null,
  "last_item_count": 10,
